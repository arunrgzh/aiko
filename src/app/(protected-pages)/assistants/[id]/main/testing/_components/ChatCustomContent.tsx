...
import { useState, useMemo, createElement, useEffect, ReactNode, Fragment, isValidElement, ReactElement, cloneElement } from 'react'
...
const AIWriter = ({
    ...
    useInterval(
        () => {
            setPos((prev) =>
                prev + 1 < totalTokens ? prev + 1 : prev
            )
        },
    ...
    }, [pos, totalTokens, onFinish])

    const wrapWithKeys = (nodes: ReactNode[]): ReactNode[] =>
        nodes.map((node, idx) => {
            if (isValidElement(node)) {
                return cloneElement(node as ReactElement, { key: idx })
            }
            return <Fragment key={idx}>{node}</Fragment>
        })

    if (Array.isArray(nodex)) {
        return <>{wrapWithKeys(nodex)}</>
    }
